...
{
  char Transmit_Buff[3];
	static int tick = 0;
  if (GPIO_Pin == GPIO_PIN_4)
  {
		if(HAL_GetTick() - tick > 50){
			HAL_UART_Transmit_DMA(&huart6,(uint8_t *)"0\n",sizeof("0\n"));
			tick = HAL_GetTick();
		}
  }
    ...
    if(HAL_GetTick() - tick > 50)
		{
			...
			{
				case 1: 
					HAL_UART_Transmit_DMA(&huart6,(uint8_t *)"1\n",sizeof("1\n"));
					break;
				case 2:
...
//				default :
	...
	if (GPIO_Pin == GPIO_PIN_15)
  {
    if(HAL_GetTick() - tick > 50)
		{
			Motor_PWM_Set(&xMotor, 0);
			Motor_PWM_Set(&yMotor, 0);
			tick = HAL_GetTick();
		}
  }
